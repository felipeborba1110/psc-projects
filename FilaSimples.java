...
public class FilaSimples {
    ...
    private static void excluir() {
        Scanner input = new Scanner(System.in);
        System.out.println("Deseja realmente excluir o primeiro valor adicionado?\nIMPORTANTE: Dados apagados não podem ser restaurados!\nDigite <s/n>:");
        String decisao = input.next();
        if ("s".equals(decisao)) {
            int i = 0;
            while (i < Global3.nElem - 1) {
                Global3.LL[i] = Global3.LL[i+1];
                i++;
            }
            Global3.nElem--;
    ...
    private static void exibir() {
        ...
        for (int i = 0; i < Global3.nElem; i++){
            System.out.printf("[%d]", Global3.LL[i]);
        }
    ...
    private static void buscar() {
        ...
        int i = 0;
        while (i < Global3.nElem) {
            if (valor == Global3.LL[i]) {
